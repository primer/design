...
Placeholders are hint text placed inside of an input. They are used to help suggest waht kind of input the should be provided, but have some accessibility considerations to be aware of.
...
Placeholders are typically styled using a light text treatment to visually communicate that they are suggestions, and not pre-filled input. However, this often means they are too light to be color contrast compliant. If the placeholder color treatment is darked to be compliant, it risks being visually interpreted as pre-filled input.

Placeholder length is also contrained by the width of the input it is used in. This means important information may be truncated, especially if the user relies on an increased font size.
