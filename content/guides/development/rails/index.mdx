...
It's important to load css directly from the gem. Otherwise lots of components will have a "broken css" look.

Load it as js module into the pipeline. Update app/assets/config/manifest.js with:
```js
//= link primer_view_components.css
```

so you can use this inside the template
```erb
