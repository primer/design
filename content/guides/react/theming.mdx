...
`ThemeProvider` comes with a [default theme object](/guides/react/theme-reference) that includes colors, spacing, fonts, etc. for building applications at GitHub.

## Customizing the theme

To customize the [default theme](/guides/react/theme-reference), you can pass your custom theme to `ThemeProvider` using the `theme` prop:
...
Some components may break if your custom theme does not include all the same keys as the [default theme](/guides/react/theme-reference). For that reason, we recommend extending the default theme using [deepmerge](https://www.npmjs.com/package/deepmerge).
...
Some [system props](/guides/react/system-props) and [`sx` prop](/guides/react/overriding-styles) keys are theme-aware. For example, the `bg` prop maps to the `colors` theme key which means you can use the `bg` prop to reference values in the `colors` object:
