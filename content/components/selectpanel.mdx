...
In most scenarios, we recommend using declarative controls, except for single-select options as [demonstrated earlier](#list-single-select). However, we understand that in certain areas of the product, using declarative controls can slow down daily actions. For example, if the issue creation process involves too many steps, it may discourage customers from reporting issues.
