...
<Note>
  This documentation is for the the [draft](https://primer.style/react/storybook/?path=/story/drafts-components-selectpanel--default&globals=colorScheme:light) version that's currently being developed in React and therefore should not be confused with the [older undocumented version](https://primer.style/react/storybook/?path=/story/components-selectpanel--default&globals=colorScheme%3Alight) that has a11y problems.
