...
In case of a multi selection or a single selection with additional actions in the footer we always recommend a `cancel` and `apply` button unless you clearly [understand the accessibility implications](#cancelsubmit) of not adding this.

### Current selection

<img width="960" alt="" src="https://github.com/primer/design/assets/980622/b1c517f4-8cc0-423b-83ac-da348aec8e76" />

In order to present a clear summary of the current selection, we create a group at the top of the list.
This is visible when clearing the search input or when the select panel is reopened.
However, it's crucial we don't create this group immediately after selecting an item, as it may cause reordering of the items and potentially confuse the users.
...
<img
  width="1668"
  alt="First select panel is a branch switcher control that includes tabs, a search input, a listbox with single-select items, and a footer with an All branches link, and cancel and apply buttons. Second select panel is titled 'Show fields, with a list of about 10 multi-select items and apply and cancel buttons in the footer."
  src="https://github.com/primer/design/assets/980622/52bcce0e-bed4-44e8-83b4-8ca6bf80eb57"
/>
...
A select panel trigger can take the form of a semantic button, allowing mouse users to click on it and keyboard users to
activate it by pressing the space or enter key.

Alternatively, the select panel can be accessed through an [action menu](/components/action-menu)
item, wherein it should appear as a centered [dialog](/components/dialog) when triggered. The action menu should close as soon as the select panel is triggered.
...
On smaller breakpoints, the select panel can be transformed into either a full-screen dialog or a bottom sheet dialog. To ensure usability on touch devices, the confirmation buttons and action list items are also increased to a medium size, ensuring a sufficiently large hit target.
