...
The **select all** option can only be used when it's possible to get all available options. For instance, if the list is too big to retrieve all options at once, like when API endpoints return limited results then this feature is disabled. We should avoid overloading API endpoints with requests for all possible results at once, as it could lead to performance issues since this component lacks virtualization capabilities.
