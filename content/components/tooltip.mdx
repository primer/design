...
<Note variant="danger">
  ...
  <Text>
    ...
    with zero or little visual indicator of its existence. See [tooltip alternatives](___) before using this component.
  </Text>
</Note>
...
<DoDontContainer>
  <Do>
    ...
    />
    <Caption>Use a tooltip to on an icon button to help convey the purpose of the action</Caption>
  </Do>
  <Dont>
    <img
      width="456"
...
>
  ...
  />
  <Box as="p" mt="0">
    For interactive controls with no visible text label such as an icon button, a tooltip can be used to provide one.
    This label should be short and descriptive just as you would for a button with visible text. The `label` serves as
    the accessible name for the control utilitizing `aria-labelledby` under the hood.
  </Box>
</Box>

### Description

The `description` is intended to supplement the control's text label by utilitizing `aria-describedby` under the hood, and will be announced to a screen reader in addition to the label. Keep the description short and descriptive.
...
>
  ...
  />
  <Box as="p" mt="0">
    For buttons that have a visible label, the `description` option can be used to provide additional context.
  </Box>
</Box>
