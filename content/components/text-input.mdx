...
The input must have an accessible name that is announced by screen readers. Additional instructions or requirements must be easily discoverable by screen reader users - in general, by having them programmatically associated with the input.

If the text input is in error/invalid, this must be programmatically conveyed, and not solely rely on visual styling (such as a colored border). In addition, the form must show a descriptive error message that provides further information to the user about what the error is, and how to correct it. See [documentation for form validation](https://primer.style/ui-patterns/forms/overview#validation) for further details.
...
In the Rails version, if a `caption` has been provided, it is correctly associated with the `<input>` using `aria-describedby`.

In the Rails version, if a `validation_message` is provided, it is correctly associated with the `<input>` using `aria-describedby`.

If the text input is set to `disabled`, the rendered `<input>` includes the `disabled` attribute.
...
- The component renders a `<label>` element, containing the `name`, and an `<input>`; the `<label for="…">` attribute must point to the `id` of the `<input>`
