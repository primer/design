...
There are different entry points to menus:

- **Icon button**: Shows items directly related to the current view.
- **Button (selection)**: Shows items directly related to the button, with a title prefix, like "Sort by: Title."
- **Right clicking list items (context menu)**: Shows items directly related to the right clicked item. For accessibility this is often also provided through an [icon button](/components/icon-button).
...
The focus should always remain on menu items, and the menu shouldn't include additional form elements like `button`, `input`, or `checkbox`.
