...
- **Description:** A brief description to adds more context for the table when the title alone is not enough
...
The table must have a title, and can optionally have a description to describe more context. If you encounter a case where there is enough context that the table doesn’t need a visible label, you may hide it so it’s only accessible to assistive technologies.

<Box display="flex" flexDirection={['column', 'column', 'column', 'column', 'row']} mb={3} sx={{gap: 3}}>

<Box as="p" flex={1}>
  If you use the table header, the table's title and description will automatically be associated with the table for
