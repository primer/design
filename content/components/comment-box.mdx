...
A [banner message](/components/banner) is used to inform users about an event related to the comment box. Events include but are not limited to error or warning messages, uploading state or successful actions. The banner is shown undertneath the comment box.
