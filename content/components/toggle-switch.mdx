...
When the UI is rendered on a touch-screen device, ensure that the toggle switch has a large enough touch target size. The toggle switch should react when the user taps or hovers anywhere in the touch target area, even if it isn't directly on the control.
