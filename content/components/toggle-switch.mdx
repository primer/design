...
A toggle switch should never be used as a replacement for a checkbox. See the [saving patterns](/ui-patterns/saving) for more details.
...
<Box mb={3} display="flex" alignItems="flex-start" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    <Box as="p" mt="0">The <a href="/ui-patterns/progressive-disclosure">progressive disclosure pattern</a> may be used to hide or show content based on whether a toggle switch is on.</Box>
    <img
        width="456"
        role="presentation"
