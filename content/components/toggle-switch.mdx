...
<!-- ![A toggle switch annotated to show the parts: knob, track, and status text](https://user-images.githubusercontent.com/2313998/159587114-97d7de3c-9ff6-4f13-beca-6e4da2d548c6.png) -->

<Box display="flex" alignItems="flex-start" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    <Box as="ul" m="0" p="0">
        <li><strong>Track</strong>: The clickable area where the toggle switch moves back and forth</li>
        <li><strong>Knob</strong>: Changes color and position depending on whether the switch is on or off</li>
        <li><strong>Status</strong>: A text label showing whether the switch is on or off</li>
    </Box>
    <img
        width="456"
        role="presentation"
        src="https://user-images.githubusercontent.com/2313998/159587114-97d7de3c-9ff6-4f13-beca-6e4da2d548c6.png"
    />
</Box>
...
<Box mb={5} display="flex" alignItems="flex-start" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    ...
    <Box as="p" mt="0">Labels should be succinct, and can describe a object or an action (for example: "Discussions" or "Automatically watch repositories"). Don't use phrases that describe the switch's state in your label.</Box>
    </div>
    <img
        width="456"
        role="presentation"
...
If you have a section of configuration options that can be disabled, use a button instead of a toggle switch. Disabling the section turns off all configuration options, but remembers their previous state when the user wants to turn the section back on. While the section is disabled, the configuration options are hidden.
...
A toggle switch should never be used as a replacement for a checkbox. See the [saving patterns](/saving) for more details.
...
<Box mb={3} display="flex" alignItems="flex-start" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    <div>
        <Box as="p" mt="0">By default, lay out a toggle switch horizontally justified with it's label and optional description.</Box>
...
<Box mb={3} display="flex" alignItems="flex-start" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    <Box as="p" mt="0">The <a href="/progressive-disclosure">progressive disclosure pattern</a> may be used to hide or show content based on whether a toggle switch is on.</Box>
    <img
        width="456"
        role="presentation"
...
Toggle switches should have the same keyboard navigation features as a button. A toggle switch may be focused using the `Tab` key, and pressing the `Enter` or `Space` key will press the button.

### Touch target

When the UI is rendered on a touch-screen device, ensure that the toggle switch has a sufficient touch target size. The toggle switch should react when the user taps or hovers anywhere in the touch target area, even if it isn't directly on the control.
