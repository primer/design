...
Default to using icon buttons with the `invisible` variant (no border/background).
...
<DoDontContainer>
  <Do>
    <img
      ...
      width="456"
    />
    {/* <!-- TODO: decide if we want to allow a danger button to be mixed into an action bar --> */}
    <Caption>Only use buttons of the same variant in an action bar, and default to using the invisible variant.</Caption>
  </Do>
  <Dont>
    <img
...
Icon buttons are the default, but buttons may also have text labels. All buttons in an action bar should have the same type of content (icon, text, or icon with text).

If the buttons use text labels and horizontally overflow the container, the overflow menu will still be triggered by an icon button with an ellipsis icon.

<DoDontContainer>
  <Do>
    <img
      src="https://github.com/primer/design/assets/2313998/522486fd-7197-497f-9c2b-ee558aec6c74"
      role="presentation"
      width="456"
    />
  ...
  <Dont>
    <img
      src="https://github.com/primer/design/assets/2313998/8c53fcad-3f36-40d5-8dea-9ccfcab8b9c2"
      role="presentation"
      width="456"
    />
