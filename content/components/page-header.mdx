...
By default, the [Context bar](#context-bar) is visible on narrow viewports only, the [Title bar](#title-bar) is visible on all viewports, and the [Back button and Trailing action](#title-bar-action-regions) are visible on regular viewports.
...
The Context bar is intended to set context for the user on smaller viewports and provide any actions that fit within the Context bar. By default, the Context bar (including its actions) is only visible on narrow viewports. 
...
On narrow viewports, the Parent link touch target expands beyond its element boundaries.
...
On narrow viewports, all title variants appear as medium.
...
On narrow viewports, all visual regions remain visible by default.
...
On narrow viewports, all action regions are hidden by default. A Back button should appear as a Parent link. Any Trailing action should be hidden in an overflow button on the side.
