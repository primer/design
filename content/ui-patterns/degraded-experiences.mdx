...
If there is a critical system error that will degrade the user experience, show a [banner](/components/banner) at the top of the page above the global navigation. Having a global banner helps set the expectation that some parts of the usual UI might be missing or broken. Default to using the `"warning"` variant of the banner.
