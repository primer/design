...
If the page is in a state where we're not sure if a link is available, the navigation item should be put in a loading state. Refer to the pattern guidelines of [loading](/ui-patterns/loading) for more information.
