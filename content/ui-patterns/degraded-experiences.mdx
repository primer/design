...
{/* If a navigation item is shown in a loading state and fails to load, don't suppress rendering of the item without informing the user that loading has failed. In the global navigation header, the global error banner will suffice. When navigation is in an overlay that obscures the global error banner (such a the left and right side sheets), place an error banner in the overlay. */}
...
<img
  ...
  src="https://github.com/primer/react/assets/2313998/da4f257f-681d-4bfa-9810-77a3391fc2ab"
/>
...
<img
  ...
  src="https://github.com/primer/react/assets/2313998/55148339-4609-4058-8c3f-e08cab5fbd1e"
/>

If sequential groups or navigation links are unable to load, replace both groups with a single loading indicator and a single error message.
...
<img
  ...
  src="https://github.com/primer/react/assets/2313998/3258b2da-2a82-4ab2-baac-2ed5aa770bc0"
/>
...
If non-sequential groups or navigation links are in a loading state, preserve the group headings and replace the contents of each group with a single loading indicator. If loading fails, show a single error message in the side-sheet.
...
<img
  ...
  src="https://github.com/primer/react/assets/2313998/eea5ec26-0145-4ec9-8f5c-409eb98a988f"
/>

If some links in the right side sheet cannot be loaded, show a banner above the [nav list](components/nav-list) notifying the user that some navigation items are unavailable.

<img
  width="960"
  alt="4 images. 1. Nav items loading - grouped items replaced with single loading state; 2: Loaded; 3. Nav items loading - sequential ungrouped items where each is replaced with a loading state; 4: Loaded"
  src="https://github.com/primer/react/assets/2313998/da9500d3-92d1-4048-96ec-ac14fc11b1c2"
/>

Default to replacing each loading navigation item with its own loading indicator. However, if a group of related links in a nav list are unable to load, replace the links with an error message but preserve the group heading.
{/* Default to replacing each loading navigation item with its own loading indicator. If a sequential navigation items have a relationship and are loaded via a single controller, replace them with an a single loading indicator. */}
