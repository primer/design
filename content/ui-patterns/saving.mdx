...
- If the save button only refers to a segment of inputs on the page, use the **secondary** button appearance.
...
<Box mb={3} display="flex" alignItems="flex-start" justifyContent="space-between" flexDirection={["column", "column", "column", "column", "row"]} sx={{gap: 3}}>
    <Box mb={3} display="flex" alignItems="center" flexDirection="column" mx="auto" sx={{gap: 2}}>
        ...
        />
        <Text as="p" mt="0" align="center" fontSize={1}>Inputs all belong to one form</Text>
        <img
            width="456"
    ...
    <Box mb={3} display="flex" alignItems="center" flexDirection="column" mx="auto" sx={{gap: 2}}>
        ...
        />
        <Text as="p" mt="0" align="center" fontSize={1}>Inputs divided into multiple sections</Text>
    </Box>
</Box>
...
<DoDontContainer>
  <Do>
    ...
    <Caption>Separate auto-saving inputs from an explicitly saved form</Caption>
  </Do>
  <Dont>
    ...
    <Caption>Don't mix auto-saving inputs and explicitly saved inputs in the same form</Caption>
  </Dont>
</DoDontContainer>
...
To ensure your forms are accessible, you should use explicit saving (and not automatic saving) in declarative inputs. These are:
...
There are a few issues to bear in mind with autosaving declarative inputs.
...
- Checkbox groups: It could be unclear whether or not the selections has been saved. Users may accidentally submit a selection clicked by accident.
...
Automatically saving a single-select dropdown menu is accessible as long as it is a semantic menu. A semantic menu can only be a list of items that can be selected.

As soon as a dropdown has additional features (search inputs, filters, tabs, etc), the dropdown should be treated like a dialog with a button to submit and a button to cancel. A dialog will provide the proper navigation features for folks using assistive technology.
...
- If the change occurred without a redirect or page refresh, you can use the toast component.
- If the change occurred and the page refreshes or redirects, you can use a flash banner component.

<Note variant="warning">If your design is being implemented in github.com, avoid using a toast for now. Those toasts have accessibility issues that need to be fixed.</Note>

## Error forgiveness

Before submitting a form with very dangerous consequences (for example, delete a repository), ask the user to confirm they want to submit the form.
