...
- [Toggle switches](/components/toggle-switch)
- Single-select dropdowns that are not native `<select>` dropdowns or part of an explicitly saved form

The toggle switch behaves like a light switch: the light just switches on and off without you having to confirm each time you flip the switch. Single-select dropdowns behave like a radio dial: the station starts playing as soon as you select it.
