...
In a few specific cases, such as updating notification preferences, we should use the `data-disable-with` property to indicate that the response is still being sent to signal to the user that they shouldn't navigate away from the current page which may cause the request to fail silently in the event of an error.
