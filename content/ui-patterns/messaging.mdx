...
Use the `info` state to highlight messaging that may help the user complete a task or provide additional context. This state is typically used for informational purposes and is not critical to the user's experience.

Examples:
- a setting that impacts the current view
- nudge towards a new feature
- a non blocking action to unlock additional features

### Warning

Use the `warning` state to inform users of a potential issue or action that may impact their experience.

Examples:
- an upcoming account setting will become required soon
- a degraded experience(link) due to connectivity or GitHub availability
- a non destructive action is about to take place

### Success

Use the `success` state to notify users that the results of an action were successful.
...
Reserve the `unavailable` state for **inline** system related degraded experiences (see docs for more information).
