...
| <Box sx={{color: 'attention.fg', display: 'flex', alignItems: 'center', gap: '0.5rem'}}><StopIcon /><span>Warning</span></Box> | Informs about a possibly unexpected result of an action they are about to take |
| <Box sx={{color: 'danger.fg', display: 'flex', alignItems: 'center', gap: '0.5rem'}}><AlertIcon /><span>Critical</span></Box> | Informs about an error that occurs or warns about loss of access or data as a result of an action |
...
Use the `critical` state to notify users that the results of an action were unsuccessful, to display form errors, or to inform that an action is destructive.
