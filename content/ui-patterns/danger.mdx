...
import Img1 from '../../images/danger/dangerPatterns1.png'
import Img2 from '../../images/danger/dangerPatterns2.png'
import Img3 from '../../images/danger/dangerPatterns3.png'
import Img4 from '../../images/danger/dangerPatterns4.png'
import Img5 from '../../images/danger/dangerPatterns5.png'
import Img6 from '../../images/danger/dangerPatterns6.png'
import Img7 from '../../images/danger/dangerPatterns7.png'
...
1. We should highlight all destructive flows visually. 
    - All page sections that will trigger a destructive sequences should be highlighted with a [`red border`](https://primer.style/css/utilities/borders#border-colors) and an [`alert` octicon](https://primer.style/octicons/alert-16) to build an accessible, intuitive pattern of recognition. When people see those details, they should know to tread carefully. Ideally, these will be centralized into a "Danger Zone" section.
...
We should force users into a secondary confirmation flow - generally a [box overlay](https://primer.style/css/components/box-overlay) - to ensure that they understand the full impact of the decision. 

1. The user should be informed of exactly which data **will** be destroyed and forced to confirm that they understand it. This should be explained clearly and concisely in a bulleted list form, using the [`x` octicon](https://primer.style/octicons/x-16) to both draw attention and emphasize that it will be deleted. We should not list out data that will *not* be deleted.
...
_All data deletion events should be logged for the support team to help them handle cases where something has gone wrong._
...
In these cases, we want to make sure users are sufficiently informed of what will happen without visually implying that data will be lost. If we are concerned that the user may not have enough context to make a decision, we should use a yellow-themed [`flash alert`](https://primer.style/css/components/box#boxes-with-flash-alerts) and the [`alert` octicon](https://primer.style/octicons/alert-16) to provide the necessary information. If an action is required, use a [`default button`](https://primer.style/css/components/buttons#default-button) or [`outline button`](https://primer.style/css/components/buttons#outline-button) instead of a `danger button`.