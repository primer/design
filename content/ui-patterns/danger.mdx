...
1. Highlight all destructive flows visually. 
    - All page sections that will trigger a destructive sequence should be highlighted with `border-red` and an `alert` icon to build an accessible, intuitive pattern of recognition. When users notice those details, they should know to tread carefully. Ideally, these will be centralized into a `Danger zone` section.
...
Take users into a secondary confirmation flow - generally a [box-overlay]() - to ensure they understand the full impact of the decision. 

1. The user should be informed of exactly which data **will** be destroyed and required to confirm they understand it. This should be explained clearly and concisely in a bulleted list form, using the [`x` octicon]() to both draw attention and emphasize that it will be deleted. Don't list out data that will *not* be deleted.
...
In these cases, make sure users are sufficiently informed of what will happen without visually implying that data will be lost. If you're concerned the user may not have enough context to make a decision, use a [`box-row`]() with `bg-yellow` and the `warning` octicon to provide the necessary information. If an action is required, use `btn` or `btn-outline` instead of `btn-danger`.
