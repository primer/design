...
As you're developing a new component, feature, or page that contains an image, use proper [semantic HTML](https://primer.style/design/accessibility/semantic-html) and use the `<img>` element with proper alternative text using the `alt` attribute.
...
- Combine `alt` text with the caption or the content. If the image is sufficiently described in the text, for example, a simple diagram illustrating what's written in the web page text, it can have brief `alt` text such as "Diagram of work flow described above".
