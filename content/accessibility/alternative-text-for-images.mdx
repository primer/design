...
Images can make the web user experience better and more pleasant, especially for users with cognitive and learning disabilities. But not all images used in websites are accessible, and that can be a major barrier. Accessible images are beneficial for everyone, especially for people using screen reader and other assistive technologies, as well as is beneficial for Search Engine Optimization (SEO). 
...
### Informative & Decorative images

To help you determine the best way to communicate the information of an image to a screen reader users you can use [the alt text decision tree provided by the W3C](
https://www.w3.org/WAI/tutorials/images/decision-tree/).

Depending the purpose of the images, they can be categorized into two categories: [informative](https://www.w3.org/WAI/tutorials/images/informative/) and [decorative](https://www.w3.org/WAI/tutorials/images/decorative/) images.
...
When you create a new design and it includes images, include captions of a description for each of them. Annote which images are informative and which ones are decorative.

### For engineers

As you're developing a new component, feature, or page that contains an image, use proper [semantic html](https://primer.style/design/accessibility/semantic-html) and use the `<img>` element with a proper alternative text using the `alt` attribute.
...
A missing alt attribute is bad, but an alt with wrong or useless information is not a better solution. When writing the alt text of an image you should always have in mind some rules:

- Keep it short. It's usually better to avoid long, boring, or irrelevant information. The alt text should be the most concise description possible of the image’s purpose.
- The alternative text should not include the words button, link, or image. Screen readers already provide that information.
- In [textual images](https://www.w3.org/WAI/tutorials/images/textual/
), images that contains words that are important to understanding the content, the words are included in the text alternative.
...
- Combine alt text with the caption or the content. If the image is sufficiently described in the text — for example, a simple diagram illustrating what's written in the web page text — it can have brief alt text such as "Diagram of work flow as describe above".
...
- [Deep Dive: Making your images accessible](https://github.com/github/accessibility/blob/main/docs/deep-dive-notes/2022-01-26-images.md)
