...
## Accessibility
When designing with colors there consider the following to make your UI visually accessible.

### Assure adequat color contrast
When combining colors like foreground and background colors, you must make sure that they fulfill the [required wcag contrast ratio](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html). 

The contrast requirements are:
- 4.5:1 for normal text
- 3:1 for large text (>24px)
- 3:1 for UI elements and graphics
- decorational and disabled elements have no contrast requirement

It's best to use an [online contrast checker](https://webaim.org/resources/contrastchecker/) or a [figma plugin](https://primer.style/design/accessibility/tools#contrast-plugin) for that. If you work with the primer primitives, this work has already been done for you. We checked all recommended combinations, so you are all set.
...
[Picture of innput field with error message and icon]

For charts and graphs you can position the labels on top or close to each section. Alternativly you can use patterns to distinguish different parts.

[Picture of line chart with different line styles and point styles]
