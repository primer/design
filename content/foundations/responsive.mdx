...
A common example of a hit target that may not meet the AAA standard on mobile is our medium size button, which is visually only 32px in height. To improve the button's touch target size without viusually adjusting its appearance, the touch target area could be increased by adjusting the button's padding. This will increase the button's touch target's overall area without needing to adjust the button's visual size.
