...
A common example of a hit target that may not meet the AAA standard on mobile is our medium size button, which is only 32px in height. To improve its touch target, the height could be increased by adjusting its padding. This will increase the touch target's overall size without altering the visual appearance of the button.
