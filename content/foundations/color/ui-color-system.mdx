...
When designing and building UI, reach for functional color tokens first. Tokens with the `default` suffix are the standard values, while `muted` and `emphasis` each impact the hierarchy respectively.
...
<img width="960" alt="wip" src="https://github.com/primer/primitives/assets/18661030/292d4828-5a30-45b6-9c9c-8cf582ced9cf" />
...
Accents colors are typically used to communicate status, action, or emphasis. Each accent color is tied to a [role](#role) with a specific meaning. Color tokens are available for foreground, background, and border. Background and border colors have both a `muted` and `emphasis` option.
